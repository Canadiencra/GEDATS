...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_societe && $adresse && $nature_produit && $marche_destination && $date_enregistrement) {
        $stmt = $pdo->prepare("INSERT INTO etablissements (id_societe, adresse, nature_produit, marche_destination, date_enregistrement) 
                               VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$id_societe, $adresse, $nature_produit, $marche_destination, $date_enregistrement]);
        $success = "✅ Établissement enregistré avec succès.";
    } else {
...
<div class="container mt-4">
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white d-flex justify-content-between">
            <h5>🏭 Ajouter un Établissement</h5>
